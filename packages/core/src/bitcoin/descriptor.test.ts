...
import { deriveAddress, normaliseExtendedKey, normaliseInput, parseDescriptor } from './index';
...
describe('normaliseExtendedKey', () => {
  it('converts zpub to the account xpub and remembers what it implied', () => {
    expect(normaliseExtendedKey(bip84.zpub)).toEqual({ xpub: bip84.xpub, network: 'mainnet', implies: 'wpkh' });
  });
  it('passes xpub through with no implication', () => {
    expect(normaliseExtendedKey(bip84.xpub)).toEqual({ xpub: bip84.xpub, network: 'mainnet' });
  });
  ...
  it('rejects garbage and private keys', () => {
    ...
    expect(() => normaliseExtendedKey('xprv9s21ZrQH143K3QTDL4LXw2F7HEK3wJUD2nW2nRk4stbPy6cq3jPPqjiChkVvvNKmPGJxWUtg6LnF5kejMRNNU3TGtRBeJgk33yuGBxrMPHi')).toThrow(/private key/);
  });
});
...
describe('normaliseInput', () => {
  ...
  it('bare xpub asks for a script type, then accepts one', () => {
    expect(normaliseInput(bip84.xpub).needsScriptType).toEqual({ xpub: bip84.xpub, network: 'mainnet' });
    expect(normaliseInput(bip84.xpub, 'tr').descriptor?.canonical).toBe(`tr(${bip84.xpub}/0/*)`);
  });
  it('accepts a full descriptor with origin, checksum and <0;1> multipath, keeping only the external chain', () => {
    const r = normaliseInput(`wpkh([73c5da0a/84h/0h/0h]${bip84.zpub}/<0;1>/*)#abcd1234`);
    expect(r.descriptor?.canonical).toBe(`wpkh([73c5da0a/84h/0h/0h]${bip84.xpub}/0/*)`);
  });
});

describe('parseDescriptor', () => {
  it('parses sortedmulti but marks it not issuable', () => {
    const p = parseDescriptor(`wsh(sortedmulti(2,${bip84.xpub}/0/*,${bip86.xpub}/0/*))`);
  ...
  it('rejects legacy and unknown forms with a clear message', () => {
    expect(() => parseDescriptor(`pkh(${bip84.xpub}/0/*)`)).toThrow(/Only wpkh/);
    expect(() => parseDescriptor('nonsense')).toThrow(/not a descriptor/);
    expect(() => parseDescriptor(`wpkh(${bip84.xpub}/1/*)`)).toThrow(/followed by \/0\/\*/);
    expect(() => parseDescriptor(`wsh(sortedmulti(3,${bip84.xpub}/0/*,${bip86.xpub}/0/*))`)).toThrow(/Threshold/);
  });
});
...
describe('deriveAddress', () => {
  ...
  it('refuses multisig and bad indices', () => {
    const p = parseDescriptor(`wsh(sortedmulti(1,${bip84.xpub}/0/*,${bip86.xpub}/0/*))`);
    expect(() => deriveAddress(p, 0)).toThrow(/not enabled/);
    expect(() => deriveAddress(normaliseInput(bip84.zpub).descriptor!, -1)).toThrow(/range/);
  });
});
