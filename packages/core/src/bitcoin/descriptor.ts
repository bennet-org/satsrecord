...
const CANONICAL_VERSION: Record<Network, string> = {
  mainnet: "0488b21e",
  testnet: "043587cf",
};
...
export function normaliseExtendedKey(input: string): ExtendedKeyInfo {
  ...
  try {
    ...
    throw new DescriptorError(
      "not_extended_key",
      "That does not look like an extended public key.",
    );
  }
  if (raw.length !== 78)
    throw new DescriptorError(
      "not_extended_key",
      "Extended key has the wrong length.",
    );
  const version = hex(raw.slice(0, 4));
  if (PRIVATE_VERSIONS.has(version) || raw[45] === 0x00)
    throw new DescriptorError(
      "private_key",
      "That is a private key. Only ever paste the public key.",
    );
  const v = VERSIONS[version];
  if (!v)
    throw new DescriptorError(
      "unknown_prefix",
      "Unrecognised extended key prefix.",
    );
  const canonical = new Uint8Array(raw);
  canonical.set(fromHex(CANONICAL_VERSION[v.network]), 0);
  return {
    xpub: b58.encode(canonical),
    network: v.network,
    ...(v.implies ? { implies: v.implies } : {}),
  };
}
...
const KEY_RE =
  /^(\[[0-9a-fA-F]{8}(?:\/\d+['h]?)*\])?([xyztuv]pub[1-9A-HJ-NP-Za-km-z]+)(\/(?:0|<0;1>)\/\*)$/;
...
function parseKey(s: string): { key: DescriptorKey; network: Network } {
  ...
  if (!m)
    throw new DescriptorError(
      "bad_key",
      `Key expression must be an extended public key followed by /0/* or /<0;1>/* (got "${s.trim().slice(0, 24)}…").`,
    );
  const info = normaliseExtendedKey(m[2]!);
  return {
    key: { ...(m[1] ? { origin: m[1] } : {}), xpub: info.xpub },
...
export function parseDescriptor(input: string): ParsedDescriptor {
  ...
  if ((m = /^wsh\(sortedmulti\((\d+),(.+)\)\)$/.exec(s))) {
    ...
    if (parts.length < 2)
      throw new DescriptorError(
        "multisig_keys",
        "Multisig needs at least two keys.",
      );
    if (threshold < 1 || threshold > parts.length)
      throw new DescriptorError(
        "multisig_threshold",
        `Threshold must be between 1 and ${parts.length}.`,
      );
  ...
  if (/^(pkh|sh\(pkh|combo|addr|raw|multi\(|wsh\(multi\()/.test(s)) {
    throw new DescriptorError(
      "unsupported_script",
      "Only wpkh, sh(wpkh), tr and wsh(sortedmulti(...)) descriptors are supported.",
    );
  }
  throw new DescriptorError(
    "not_descriptor",
    "That is not a descriptor we recognise.",
  );
}

function single(
  scriptType: Exclude<ScriptType, "wsh_sortedmulti">,
  keyExprIn: string,
): ParsedDescriptor {
...
function sameNetwork(ns: Network[]): Network {
  const first = ns[0]!;
  if (ns.some((n) => n !== first))
    throw new DescriptorError(
      "mixed_networks",
      "All keys must be for the same network.",
    );
  return first;
}
...
export function normaliseInput(
  ...
  if (/^[xyztuv]pub[1-9A-HJ-NP-Za-km-z]+$/.test(s)) {
    ...
    if (!st)
      return { needsScriptType: { xpub: info.xpub, network: info.network } };
    const wrapped =
      st === "wpkh"
        ? `wpkh(${info.xpub}/0/*)`
        : st === "sh_wpkh"
