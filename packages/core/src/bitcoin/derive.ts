...
import { DescriptorError, parseDescriptor, type ParsedDescriptor } from './descriptor';
...
export function deriveAddress(parsed: ParsedDescriptor, index: number): string {
  if (!Number.isInteger(index) || index < 0 || index >= 0x80000000) throw new DescriptorError('bad_index', 'Index out of range.');
  if (!parsed.issuable) throw new DescriptorError('not_issuable', 'Multisig issuance is not enabled yet.');
  const key = parsed.keys[0]!;
  const pub = HDKey.fromExtendedKey(key.xpub, parsed.network === 'mainnet' ? undefined : { private: 0x04358394, public: 0x043587cf })
    .deriveChild(0)
