...
describe("crypto", () => {
  ...
  it("email index is keyed and normalised", () => {
    const c = createCrypto(keys);
    expect(c.emailIndex(" Donor@Example.org ")).toBe(
      c.emailIndex("donor@example.org"),
    );
    expect(c.emailIndex("a@example.org")).not.toBe(
      c.emailIndex("b@example.org"),
    );
    const other = createCrypto({
      ...keys,
      indexKey: randomBytes(32).toString("base64"),
    });
    expect(other.emailIndex("a@example.org")).not.toBe(
      c.emailIndex("a@example.org"),
    );
  });
  it("refuses bad keys", () => {
    expect(() =>
      createCrypto({ encryptionKey: "short", indexKey: keys.indexKey }),
    ).toThrow(/32 bytes/);
    expect(() =>
      createCrypto({
        encryptionKey: keys.encryptionKey,
        indexKey: keys.encryptionKey,
      }),
    ).toThrow(/differ/);
  });
});
