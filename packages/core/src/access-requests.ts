...
const countryLabel: Record<AccessRequestCountry, string> = {
  GB: "United Kingdom",
  US: "United States",
  EU: "EU member state",
  other: "Other",
...
export function accessRequestNotification(
  ...
  const html = `<h2>New access request</h2><table>${rows
    .map(
      ([k, v]) =>
        `<tr><th align="left" style="padding:4px 12px 4px 0">${k}</th><td style="padding:4px 0;white-space:pre-wrap">${escapeHtml(v)}</td></tr>`,
    )
    .join("")}</table>`;
...
export async function submitAccessRequest(
  ...
  if (mail.notify) {
    ...
    await mailer.send({
      ...
      replyTo: input.email,
      ...n,
      templateVersion: ACCESS_REQUEST_NOTIFY_TEMPLATE,
    });
    await db
