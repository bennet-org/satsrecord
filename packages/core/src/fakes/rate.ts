...
export class FixedRateSource implements RateSource {
  constructor(private readonly rates: Record<string, number> = { USD: 65_216, GBP: 48_992, EUR: 56_310 }) {}
  async rateAt(currency: string, at: Date) {
    ...
    return { rate, source: 'fixed', pair: `XBT/${currency.toUpperCase()}`, method: 'fixed-demo-rate', timestamp: at };
  }
}
