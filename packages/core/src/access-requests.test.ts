...
const input = { organisation: 'Example Trust', name: 'A. Person', email: 'a@example.org', country: 'GB' as const, message: 'Hello <b>there</b>' };
...
describe('submitAccessRequest', () => {
  it('stores the row and notifies when a notify address is set', async () => {
    ...
    const { id } = await submitAccessRequest(db, mailer, input, { from, notify: 'team@satsrecord.org' });

    const rows = await db.select().from(accessRequests);
    expect(rows).toHaveLength(1);
    expect(rows[0]).toMatchObject({ id, organisation: 'Example Trust', status: 'new', website: null });
    expect(rows[0]!.notifiedAt).toBeInstanceOf(Date);

    expect(mailer.sent).toHaveLength(1);
    expect(mailer.sent[0]).toMatchObject({ to: 'team@satsrecord.org', replyTo: 'a@example.org', subject: 'Access request: Example Trust' });
