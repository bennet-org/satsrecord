...
export function createCrypto(keys: {
  encryptionKey: string;
  indexKey: string;
}): Crypto {
  const enc = keyFromBase64(keys.encryptionKey, "ENCRYPTION_KEY");
  const idx = keyFromBase64(keys.indexKey, "INDEX_KEY");
  if (timingSafeEqual(enc, idx))
    throw new Error("ENCRYPTION_KEY and INDEX_KEY must differ");
  ...
  return {
    ...
    decrypt(ciphertext) {
      ...
      if (v !== VERSION || !ivB64 || !ctB64 || !tagB64)
