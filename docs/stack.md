...
| Hosting      | Web on Netlify (adapter switches on `NETLIFY=true`); Neon Postgres. Docker image with the node adapter for self-host | Cheapest path to a hosted demo, and nothing in the code knows the host. Data lives in Neon either way.         | Worker and node on a box (Fly or similar), web moves there or stays; Cloudflare in front |
...
- **Serverless for the worker.** The poller needs a long-running process next to a node whose RPC is not on the internet. The web app is fine on Netlify: the only host-specific line is the adapter, chosen at build time, and Better Auth's rate limiter is database-backed so it holds across function instances.
